...
def sync_output_dir(output_dir, device):
    """ 让 rank 0 的 output_dir 同步给所有进程 """     
    # 先把字符串变成字节流   
    output_dir_bytes = output_dir.encode('utf-8')    
    # 创建一个固定长度的byte tensor    
    output_dir_tensor = torch.zeros(512, dtype=torch.uint8, device=device)  
    output_dir_tensor[:len(output_dir_bytes)] = torch.tensor(list(output_dir_bytes), dtype=torch.uint8, device=device)   

    # 广播：从rank=0广播到所有进程
    dist.broadcast(output_dir_tensor, src=0)  

    # 把tensor再转回字符串
    output_dir_str = bytes(output_dir_tensor.tolist()).decode('utf-8').rstrip('\x00')     
...
class BaseSolver(object):
    ...
    def _setup(self, eval=False):
        ...
        if eval:  
            self.output_dir = Path(f'{str(Path(cfg.output_dir))}_eval')
        elif self.cfg.resume:
            self.output_dir = Path(os.path.dirname(str(self.cfg.resume)))    
        else:
            if dist_utils.is_main_process():
                self.output_dir = increment_path(cfg.output_dir)     
            else:
                self.output_dir = "tmp"
    
        if dist_utils.is_dist_available_and_initialized():
            self.output_dir = Path(sync_output_dir(str(self.output_dir), device))     

        cfg.output_dir = self.output_dir   
        print(RED  + f"output_dir:{str(self.output_dir)}" + RESET)
        if dist_utils.is_main_process():
    ...
    def state_dict(self):   
        ...
        state['name'] = self.train_dataloader.dataset.category2name   
